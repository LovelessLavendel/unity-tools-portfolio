/* =========================================================================
   PRISM.JS CORE ENGINE + C# LANGUAGE EXTENSION (COMBINED & FIXED)
   ========================================================================= */
                        ...
                        var g = o[l], c = g.inside, u = !!g.lookbehind, d = !!g.greedy, p = 0, f = g.alias; if (d && !g.pattern.global) { var h = g.pattern.toString().match(/[imsuy]*$/); g.pattern = RegExp(g.pattern.source, h + "g") } g = g.pattern || g; for (var m = 0, v = 0; m < t.length; v += t[m].length, ++m) {
                            var y = t[m]; if (t.length > e.length) break e; if (!(y instanceof a)) {
                                g.lastIndex = d ? v : 0; var k = g.exec(d ? e : y); if (k) {
                                    u && (p = k[1] ? k[1].length : 0); var b = k.index + p, k = k[0].slice(p), w = b + k.length, A = y.slice(0, b - v), x = y.slice(w - v); if (A && (t.splice(m, 1, A), ++m, v += A.length), d && w > v + y.length && (w = v + y.length, k = e.slice(b, w), x = e.slice(w)), t.splice(m, 1, new a(a, c ? r.tokenize(k, c) : k, f, k, d)), x && t.splice(m + 1, 0, x), A || x) m += x ? 1 : 0, --m, v -= A ? A.length : 0
                                }
                            }
                        }
                    }
...
/* C# GRAMMAR EXTENSION (INTEGRATED) */
